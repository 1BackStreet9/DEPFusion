...
class virRefineSingleStageDetector(BaseDetector):
    def __init__(
        ...
        spe_vi: ConfigType = None,
        spe_ir: ConfigType = None,
        ...
        if spe_vi is not None:
            self.spe_vi = MODELS.build(spe_vi)
        if spe_ir is not None:
            self.spe_ir = MODELS.build(spe_ir)
    ...
    def loss(self, batch_inputs: Tensor, batch_inputs2: Tensor, batch_data_samples: SampleList) -> Union[dict, list]:
       ...
       # tvloss = self.Total_Variation_loss(img,1)
        #print(tvloss.shape)
        #losses.update(tvloss)
        return losses
    ...
    def Total_Variation_loss(self, x, weight):
        #print(x.shape)
        h_tv = torch.pow((x[:, :, 1:, :]-x[:, :, :-1, :]), 2).mean()
        w_tv = torch.pow((x[:, :, :, 1:]-x[:, :, :, :-1]), 2).mean()
        loss = weight * (h_tv + w_tv)
        return {"tv_loss":loss}
